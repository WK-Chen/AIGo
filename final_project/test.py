...
def fetch_new_games(data_path, dataset, last_id):
    """ Update the dataset with new games from the databse """

    def key(elem):
        pattern_1 = 'id_(.*)'
        return re.findall(pattern_1, elem)

    # Fetch new games in reverse order so we add the newest games first
    added_moves = 0
    added_games = 0
    new_games = [game for game in os.listdir("./data")]
    new_games.sort(key=key, reverse=False)
    new_games = new_games[last_id:]
    # last_id = int(key(new_games[0])[0])
    logging.info("Fetching: {} new games from data_path=\"{}\"".format(len(new_games), data_path))

    for game in new_games:
        with open(data_path + '/' + game, 'rb') as f:
    ...
    logging.info("Last id: {}, added games: {}, added moves: {}".format(last_id, added_games, added_moves))
